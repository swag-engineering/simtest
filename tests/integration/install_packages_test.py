...
@pytest.mark.parametrize("package_name", ["beautifulsoup4", "requests", "httpie"], indirect=True)
async def test_install_valid_packages(
        package_name: str,
        package_bytes: bytes,
        simtest_client: proto.grpc.SimtestStub
):
    request = proto.grpc.InstallWheelPackageRequest(package_name, package_bytes)
...
async def test_install_invalid_packages(
    ...
    request = proto.grpc.InstallWheelPackageRequest("flask", package_bytes)
